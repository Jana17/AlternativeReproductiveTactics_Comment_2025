...
namespace ART_sim_code {
    ...
    struct Param {
        ...
        bool evolving_threshold = false; //can be set to true or false
    ...
    void reproduce(std::vector<Individual>& pop_females,
        ...
        //original version of the mating system:
        double prop_minor_males = static_cast<double>(mature_minor_male_IDs.size()) / NrMatureMales;
        double prob_mating_minor = prop_minor_males * parameters.ART_success;
        if (mature_major_male_IDs.size() == 0) { prob_mating_minor = 1; } //If there are no major males, ALL females mate with minor males!
        ...
        for (size_t i = 0; i < pop_females.size(); i++) {
            if (pop_females[i].IsMature) {//only mature females
                ...
                //Original version of the mating system
                if (rng.bernouilli(prob_mating_minor)) {
                    size_t MinorMale_Pos = rng.random_number(mature_minor_male_IDs.size());
                    auto MinorMale_ID = mature_minor_male_IDs[MinorMale_Pos];
    ...
    void run() {
        ...
        std::ofstream ofs1("ART_Fig1_" + TEvoString + "_initT" + std::to_string(parameters.initi_threshold) + ".csv");
